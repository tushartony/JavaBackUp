...
//	public static int count = 0;
			...
			) throws Exception
	{
		...
		@Produces("text/html")
		public Response SignUp(@FormDataParam("UserName") String  UserName,@FormDataParam("UserPassword") String  UserPassword) throws UnknownHostException {
			
			System.out.println("User is is :"+UserName+" ***** File name is :"+UserPassword);
			
			
			MongoClient mongoClient = new MongoClient("localhost", 27017);
			DB mongoDB = mongoClient.getDB("UserSignUpInfo");

			//Let's store the standard data in regular collection
			DBCollection collection = mongoDB.getCollection("Usr");
			...
			if (!cursor.hasNext()) {
				...
				document.append("Password", UserPassword);
				...
				logger.info("fileInputDetails: " + UserPassword);

				
				
			} else {
				 status = "Unable to insert record with ID: " + UserName +" as record already exists!!!";
				//logger.info(status);
